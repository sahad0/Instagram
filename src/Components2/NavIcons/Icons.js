...
export default function Icons({home,Home,message,Message,post,Post,people,People,activity,Activity,checkTrue}) {
  ...
  const content = (
    ...
    renderItem={(item) => (
      <List.Item >
        <List.Item.Meta
          avatar={<Avatar className='my-2' src={item.av} />}
          title={<div style={{display:"flex",justifyContent:"space-between"}}><a className='my-3' href="https://ant.design">{item.name}<span style={{fontSize:"12px",color:"gray"}}>  liked your post </span></a> <img className='img-fluid' style={{height:"35px",width:"35px"}} src={item.pic} alt="pic" /></div>}
