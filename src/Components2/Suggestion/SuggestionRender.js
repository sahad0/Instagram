        ...
        return suggest.map((item, index) => {
            ...
            return (
                ...
                <div key={index} style={{ height: "20%", backgroundColor: "#FAFAFA" }} >
                    <List.Item >
                        <List.Item.Meta
                            avatar={<Avatar src={fetchImg(item.source)} />}
                            title={<a href="#" onClick={() => { ProfileView(item.name, fetchImg(item.source)) }} style={{ fontWeight: "bold", fontSize: "13px" }}>{item.name}</a>}
